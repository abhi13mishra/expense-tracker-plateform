...
const BASE_URL = "https://expense-tracker-plateform-1.onrender.com/api";
...
const Profile = ({ onUpdateProfile, onLogout }) => {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (data) {
        const updatUser = data.user || data;
        setUser(updatUser);
        setTempUser(updatUser);
        setEditMode(false);

        onUpdateProfile?.(updatUser);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      //reset password visiblitity
      setShowPassword({ current: false, new: false, current: false });
    } catch (error) {
